...
classDiagram
    ...
    note for AgentState "memory: Phase 7~\nlogs: Phase 6~"
```
...
flowchart LR
    ...
    Phase1 --> Phase15 --> Phase2 --> Phase3 --> Phase4 --> Phase5 --> Phase6 --> Phase7
```
...
```
Phase 1.5完了後（現在地）
  → Phase 2: セルフチェック（プランの品質検証）
  → Phase 3: LangGraph（フロー制御移行 + セルフチェックの分岐）
