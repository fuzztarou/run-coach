...
│   ├── database.py        # PostgreSQL 操作（upsert, save_splits等）
│   ├── workout_store.py   # ワークアウト保存ノード（LangGraph）
│   ├── converters.py      # ペース単位変換ユーティリティ
│   ├── feedback_parser.py # Garmin description パース
│   ├── api.py             # FastAPI アプリケーション
│   ├── auth.py            # OIDC トークン検証
│   ├── cloud.py           # Cloud Run 環境判定
│   ├── gcs.py             # GCS ファイル操作
│   ├── line.py            # LINE 通知機能
│   └── look_back.py       # 振り返り対話ロジック
├── tests/                 # テスト
├── docs/                  # Phase毎の詳細設計
├── config/                # 設定ファイル
...
| 個人スケジュール | Google Calendar API | ローカルはOAuth、Cloud Runはサービスアカウント共有カレンダー |
...
flowchart TB
    ...
    subgraph エージェント["エージェント (LangGraph)"]
        ...
        DB[PostgreSQL<br>workouts / workout_splits]
...
| **5.1** | Upsert対応: ワークアウト重複保存の防止 | [docs/phase5.1-upsert.md](docs/phase5.1-upsert.md) |
...
class WorkoutSummary(BaseModel):
    ...
    garmin_activity_id: str | None  # Phase 4で追加
    description: str | None         # Phase 4で追加（Garminメモ）
...
class WorkoutPlan(BaseModel):
    ...
    intensity: str | None = "low"  # low, moderate, high
    max_hr: int | None = None    # 心拍上限（LLMが年齢・目的から決定。restはnull）
    notes: str | None = ""
...
```json
{
  ...
  "workout_evaluation": "先週はイージーラン3回+ロング走1回。ロング走で心拍が高めだった。",
  "workouts": [
  ...
  "reasoning": "先週のロング走で心拍が高めだったため、今週は高強度を1回に抑えテンポ走に集中。"
}
```
...
## Human-in-the-loopのログ設計（Phase 4）

当初はDecision/Outcomeの2テーブル構成を計画していたが、実装では `workouts` テーブルに振り返りカラム（`rpe`, `pain`, `comment`, `look_back_prompted_at`）を追加する方式を採用した。

| データ | 保存先 | 備考 |
|--|--|--|
| ワークアウト実績 | `workouts` テーブル | Garminから取得、upsert |
| ラップデータ | `workout_splits` テーブル | 1km毎のスプリット |
| 振り返り（RPE・痛み・コメント） | `workouts` テーブルのカラム | LINE対話で入力、同行に保存 |

将来的にDecision（週次プラン + 根拠）を別テーブルで管理する構想は残しているが、現時点では未実装。

## LangGraph設計方針（Phase 3で移行済み）

各処理を state(AgentState) を受け取り state を返す関数にし、LangGraphのノードとして構成している。

- **各処理がLangGraphノード** — `fetch_workouts`, `save_workouts`, `fetch_races`, `fetch_calendar`, `fetch_weather`, `generate_plan`, `self_check`, `output_plan`, `sync_calendar`, `notify_line`
...
graph.add_node("save_workouts", save_workouts)      # Phase 4.5で追加
...
graph.add_node("notify_line", notify_line)           # Phase 7で追加

# fetch_workouts → save_workouts → fetch_races → ... → generate_plan → self_check
...
# output_plan → sync_calendar → notify_line → END
```
...
| 6章 Advanced RAG | 後回しOK | RAGを再開するときに戻る |
...
### Phase 4: Garmin descriptionから取得

```
ラン後 → ユーザーがGarminアプリでメモ入力（description）
  ↓
CLI実行時にワークアウトと一緒にdescriptionを取得
  ↓
パースしてDBに保存（Phase 4時点ではSQLite、Phase 5以降はPostgreSQL）
```
...
| 保存 / 利用方法 | データ | 理由 |
|--|--|--|
| 通常のDB検索 | ワークアウトログ、ラップ、振り返り、将来のoutcomes | 現時点では構造化データとして扱う方針 |
| 将来検討 | 自由文メモの検索 | 必要になった時点でRAGを再検討 |
...
```
Phase 1〜5: 完了
Phase 6.1: 完了（Dockerfile + ローカルDocker実行）
Phase 6.2: 完了（Cloud Run デプロイ + Secret Manager）
Phase 6.3: 完了（Cloud Scheduler + 自動実行）
Phase 7: 完了（LINE通知 + 振り返り対話）
─── 現在地 ───
...
| 環境 | 方法 | 備考 |
|--|--|--|
| ローカル (Phase 1〜5) | **`.zprofile` に環境変数定義** | シェル起動時に自動読み込み |
| 本番 (Phase 6: Cloud Run) | **GCP Secret Manager** | Terraform等で管理 |
...
| Google Calendar OAuthトークン | 中（スケジュール閲覧） | ローカル開発時のみ利用 |
...
| HRVトレンド（improving/declining） | o（※現時点では未取得） | |
| GPS座標・位置情報 | | x |
| 氏名・メールアドレス | | x |
| Garmin生データ（巨大JSON） | | x |
...
- ローカルPostgreSQLの認証情報を安全に管理する
- `.gitignore` に `*.db`, `*.sqlite`, `.env` を追加

### Cloud Run のアクセス制御（Phase 6）
