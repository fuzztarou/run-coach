...
class Plan(BaseModel):           # LLM出力（JSON）
    week_start: str
    days: list[dict]             # date, workout_type, duration, intensity, notes
    load_summary: str
...
```json
{
  ...
  "days": [
    {"date": "2026-03-09", "workout_type": "easy_run", "duration_min": 40, "intensity": "low", "notes": ""},
    {"date": "2026-03-11", "workout_type": "tempo", "duration_min": 50, "intensity": "high", "notes": "4:30/kmで20分"},
    {"date": "2026-03-13", "workout_type": "easy_run", "duration_min": 40, "intensity": "low", "notes": ""},
    {"date": "2026-03-15", "workout_type": "long_run", "duration_min": 90, "intensity": "moderate", "notes": "LSD 15km"}
  ],
...
| 環境 | 方法 | 備考 |
|--|--|--|
| ローカル (Phase 1〜4) | **1Password CLI (`op`)** | `.env` に平文で置かない |
| 本番 (Phase 5: Cloud Run) | **GCP Secret Manager** | Terraformで管理 |

### ローカル: 1Password CLI

```bash
# .env.tpl（シークレット参照のみ。gitにコミット可能）
GARMIN_EMAIL=op://Personal/Garmin/email
GARMIN_PASSWORD=op://Personal/Garmin/password
OPENAI_API_KEY=op://Personal/OpenAI/api_key
ANTHROPIC_API_KEY=op://Personal/Anthropic/api_key
OPENWEATHERMAP_API_KEY=op://Personal/OpenWeatherMap/api_key
```

```bash
# 実行時に1Passwordから取得して環境変数に注入
op run --env-file=.env.tpl -- python run_coach.py
```
